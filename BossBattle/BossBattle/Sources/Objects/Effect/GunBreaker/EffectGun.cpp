...
EffectGun::EffectGun(std::shared_ptr<ObjectManager> objectManager, std::shared_ptr<BaseCharacter> _player)
	: BaseEffect(),
	useChara(_player)
...
void EffectGun::Draw(ComPtr<ID3D11DeviceContext> pDeviceContext)
{
...
void EffectGun::DrawSet(ComPtr<ID3D11DeviceContext> pDeviceContext)
{
	float scale = 1.0f;
	...
	DirectX::XMVECTOR Color = DirectX::XMVectorSet(0.0f, 0.0f, 0.2f, 1.0f);
	DirectX::XMVECTOR EdgeColor = DirectX::XMVectorSet(1.0f, 0.0f, 0.0f, 1.0f);
...
void EffectGun::OnCollisionEnter(ObjectTag _tag, DirectX::XMFLOAT3 delta)
{

}

bool EffectGun::IsNewHit(BaseObject* obj)
{
